...
import { Container, Title, Paper, Text, Skeleton, SegmentedControl, Group, SimpleGrid, List, ThemeIcon, Button, Table, Grid, Stack } from '@mantine/core';
...
function DashboardPage() {
  ...
  const topSalespeopleRows = salespersonRanking.map((salesperson) => (
    <Table.Tr key={salesperson.salesperson_name}>
      ...
      <Table.Td align="left">{salesperson.total_sales}</Table.Td>
      <Table.Td align="left">{formatCurrency(salesperson.total_value)}</Table.Td>
    </Table.Tr>
  ));
    ...
    <Stack gap="md">
      ...
      <Grid>
        {can('quotes.view') && (
          <Grid.Col span={{ base: 12, md: 6, lg: 6 }}>
            <Paper withBorder p="lg" h="100%">
              ...
              <BarChart h={300} data={quoteStatusData} dataKey="status" series={[{ name: 'Orçamentos' }]} tooltipProps={{ content: ({ label, payload }) => <ChartTooltip label={label} payload={payload} />, cursor: false }} xAxisProps={{ angle: 0, textAnchor: 'middle', fontSize: 10 }}>
        ...
        {(can('production_orders.view') || can('production_orders.view_all')) && (
          <Grid.Col span={{ base: 12, md: 6, lg: 6 }}>
            <Paper withBorder p="lg" h="100%">
              ...
              <BarChart h={300} data={orderStatusData} dataKey="status" series={[{ name: 'Pedidos' }]} tooltipProps={{ content: ({ label, payload }) => <ChartTooltip label={label} payload={payload} />, cursor: false }} xAxisProps={{ angle: 0, textAnchor: 'middle', fontSize: 10 }}>
        ...
        {can('quotes.view_all') && salespersonRanking.length > 0 && (
          <Grid.Col span={{ base: 12, md: 6, lg: 6 }}>
            <Paper withBorder p="lg" h="100%">
              <Title order={3} mb="md">Ranking de Vendedores</Title>
              <Table>
                <Table.Thead>
                  <Table.Tr>
                    <Table.Th>Vendedor</Table.Th>
                    <Table.Th align="right">Vendas</Table.Th>
                    <Table.Th align="right">Valor Total</Table.Th>
                  </Table.Tr>
                </Table.Thead>
      ...
      {(can('stock.manage') && lowStockProducts.length > 0) || (can('quotes.view') && staleQuotes.length > 0) ? (
        <Paper withBorder p="lg" mb="md">
          ...
          {can('stock.manage') && lowStockProducts.length > 0 && (
            <List spacing="xs" size="sm" center mt="md" icon={ <ThemeIcon color="red" size={24} radius="xl"><IconAlertTriangle size={16} /></ThemeIcon> }>
